...
const knockout: Match[] = [
  ...
  // ── 3位決定戦（7/18マイアミ）── フランス 4-6 イングランド
  // 10ゴールの乱打戦は1982年以来のW杯最多タイ。サカがハットトリック、ライス開始2分16秒の先制弾、
  // ムバッペは2発で通算22点としW杯歴代最多得点記録を更新（ベリンガム90+8'で締め）
  ko("m-3rd", "3位決定戦", "2026-07-18", "21:00", "MIA", { home: "FRA", away: "ENG" }, { homeScore: 4, awayScore: 6 }),

  // ── 決勝（7/19 ニューヨーク/ニュージャージー）現地15:00 ET → JST 7/20 04:00 ──
  // スペイン 1-0 アルゼンチン（延長）。途中出場フェラン・トレスが106分に決勝点（N.ウィリアムスのアシスト）。
  // アルゼンチンはエンソ・フェルナンデスが90+3'に2枚目の警告で退場、120分間でシュート2本・枠内0本に封じられた。
  // スペインが2010年以来2度目の優勝。男女W杯同時保持は史上初。
  ko("m-final", "決勝", "2026-07-19", "19:00", "NYNJ", { home: "ESP", away: "ARG" }, { homeScore: 1, awayScore: 0, extraTime: true }),
];
