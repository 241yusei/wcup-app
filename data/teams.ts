import { Team } from "@/lib/types";
import { briefings } from "./briefings";

// 注: 各国の戦術・選手・小ネタは編集者がキュレーションする前提のシードデータ。
// グループ分け（group）は確定情報に差し替えて運用すること。
export const teams: Team[] = [
  {
    code: "JPN",
    name: "日本",
    enName: "Japan",
    flag: "🇯🇵",
    group: "F",
    fifaRank: 18,
    themeColor: "#14224f",
    nickname: "サムライブルー",
    tactics:
      "前線からの連動プレスと素早いトランジションが武器。欧州主要リーグで主力を張る選手が増え、ボール保持とカウンターを使い分ける成熟したサッカーへ。",
    recent:
      "本大会はグループF。オランダ・スウェーデン・チュニジアと同組で、欧州の強豪オランダとの一戦が最大の山場。基本布陣は3-4-2-1。主力の三笘薫を負傷で欠くなか、久保建英を中心とした攻撃で『ジャイキリ常連』として史上初のベスト8突破を狙う。",
    players: [
      { name: "久保 建英", position: "MF", club: "レアル・ソシエダ", why: "三笘不在の今大会、攻撃の絶対軸。右から左足でカットインする仕掛けは、にわかでも一目で分かる派手さ。" },
      { name: "堂安 律", position: "MF", club: "フランクフルト", why: "背番号10の勝負師。正確な左足とドリブルで仕掛け、大舞台に強い。カタールではドイツ・スペイン撃破弾。" },
      { name: "遠藤 航", position: "MF", club: "リバプール", why: "中盤の番人にして主将。地味だが彼がボールを刈り取るから攻撃が回る『縁の下の主役』。" },
    ],
    trivia: [
      "アウェイユニフォームのコンセプトは『COLORS』。11色の途切れたピンストライプで多様性と団結を表現。",
      "ホームの青は『サムライブルー』。海と空、日本の自然を象徴すると言われる。",
    ],
  },
  {
    code: "BRA",
    name: "ブラジル",
    enName: "Brazil",
    flag: "🇧🇷",
    group: "C",
    fifaRank: 5,
    themeColor: "#f7d716",
    nickname: "セレソン",
    tactics:
      "個の技術で局面を打開する伝統に、組織的な守備とサイド攻撃を融合。前線のタレントが多く、誰が出ても得点の匂いがする。",
    recent:
      "本大会はグループC。2022年に苦しめられたモロッコと同組で、いきなり好カードが実現。優勝5回の最多記録を持つ常勝国として6回目の戴冠が悲願。",
    players: [
      { name: "ヴィニシウス・ジュニオール", position: "FW", club: "レアル・マドリード", why: "世界最速級のドリブラー。仕掛けるたびにスタジアムがどよめく華のある選手。" },
      { name: "ロドリゴ", position: "FW", club: "レアル・マドリード", why: "決定的な仕事を淡々とこなす冷静さ。大舞台に強い。" },
    ],
    trivia: [
      "サッカー王国。ユニフォームの黄色は『カナリア色』と呼ばれ国の象徴。",
      "試合前後のサンバとサポーターの熱狂は世界一の盛り上がり。",
    ],
  },
  {
    code: "ARG",
    name: "アルゼンチン",
    enName: "Argentina",
    flag: "🇦🇷",
    group: "J",
    fifaRank: 3,
    themeColor: "#75aadb",
    nickname: "アルビセレステ",
    tactics:
      "前回王者。堅い守備ブロックと勝負所での個の決定力。試合運びの巧さ（試合巧者）はトーナメントで絶大。",
    recent:
      "2022カタール優勝の主力が経験値を上げて連覇に挑む。メッシの去就と世代交代が最大の見どころ。",
    players: [
      { name: "リオネル・メッシ", position: "FW", club: "インテル・マイアミ", why: "言わずと知れたGOAT。出場するだけでその試合が『歴史』になる。" },
      { name: "フリアン・アルバレス", position: "FW", club: "アトレティコ・マドリード", why: "走り続ける献身と決定力。メッシ後を担うエース。" },
    ],
    trivia: [
      "国民の熱狂度は世界屈指。優勝時は数百万人が街に繰り出した。",
      "水色と白のストライプは空と雲を表すとされる。",
    ],
  },
  {
    code: "FRA",
    name: "フランス",
    enName: "France",
    flag: "🇫🇷",
    group: "I",
    fifaRank: 1,
    themeColor: "#1a2a6c",
    nickname: "レ・ブルー",
    tactics:
      "圧倒的な身体能力とスピードを活かした縦に速い攻撃。個のタレント総量は世界最高クラスで、守備から一瞬で得点まで到達する。",
    recent:
      "前々回王者・前回準優勝。常に優勝候補の最右翼。エムバペを中心に黄金世代が円熟期へ。",
    players: [
      { name: "キリアン・エムバペ", position: "FW", club: "レアル・マドリード", why: "世界最速のスプリント。走り出したら誰も追いつけない、W杯の主役候補No.1。" },
      { name: "オーレリアン・チュアメニ", position: "MF", club: "レアル・マドリード", why: "中盤の壁。彼が相手の攻撃を断ち切るからエムバペが活きる。" },
    ],
    trivia: [
      "多様なルーツを持つ選手が集う『多文化代表』の象徴。",
      "ニックネーム『レ・ブルー』は青の意味。",
    ],
  },
  {
    code: "ESP",
    name: "スペイン",
    enName: "Spain",
    flag: "🇪🇸",
    group: "H",
    fifaRank: 2,
    themeColor: "#c60b1e",
    nickname: "ラ・ロハ",
    tactics:
      "ボールを握り続ける『ティキタカ』の本家。短いパスをつなぎ相手を走らせて崩す。若い才能が次々台頭し攻撃が活性化。",
    recent:
      "EURO制覇など国際大会で復権。10代〜20代前半のタレントが主力化し、新たな黄金期に入りつつある。",
    players: [
      { name: "ラミン・ヤマル", position: "FW", club: "バルセロナ", why: "10代で世界を席巻する天才。年齢を感じさせないプレーに驚かされる。" },
      { name: "ペドリ", position: "MF", club: "バルセロナ", why: "ボールを失わない司令塔。試合のテンポを自在に操る頭脳。" },
    ],
    trivia: [
      "2010年大会で悲願の初優勝。パスサッカーの代名詞。",
      "日本が2022年に逆転勝ちした相手。番狂わせの記憶が新しい。",
    ],
  },
  {
    code: "ENG",
    name: "イングランド",
    enName: "England",
    flag: "🏴󠁧󠁢󠁥󠁮󠁧󠁿",
    group: "L",
    fifaRank: 4,
    themeColor: "#cf0a2c",
    nickname: "スリー・ライオンズ",
    tactics:
      "サッカー発祥国。強度の高いプレミアリーグで鍛えられた選手が揃い、堅実な守備とセットプレーの破壊力が持ち味。",
    recent:
      "本大会はグループL。2018年準決勝で敗れたクロアチアと同組で、因縁の再戦が実現。近年は安定して上位進出するが、『1966年以来の戴冠』が長年の悲願。",
    players: [
      { name: "ジュード・ベリンガム", position: "MF", club: "レアル・マドリード", why: "攻守に効く万能型。得点もアシストもこなす次世代の主役。" },
      { name: "ハリー・ケイン", position: "FW", club: "バイエルン", why: "点取り屋にして司令塔。彼のポストプレーから攻撃が始まる。" },
    ],
    trivia: [
      "サッカーの母国。『フットボールが故郷に帰る』は応援の合言葉。",
      "PK戦に弱いジンクスに長年悩まされてきた歴史も。",
    ],
  },
  {
    code: "GER",
    name: "ドイツ",
    enName: "Germany",
    flag: "🇩🇪",
    group: "E",
    fifaRank: 10,
    themeColor: "#000000",
    nickname: "ディ・マンシャフト",
    tactics:
      "組織力と効率の伝統に、若手の台頭で柔軟性が加わる。試合を支配する保持と、勝負強さを取り戻しつつある。",
    recent:
      "近年は早期敗退が続き再建期。2022年に日本に敗れた雪辱を晴らしたい優勝経験国。",
    players: [
      { name: "ジャマル・ムシアラ", position: "MF", club: "バイエルン", why: "ぬるぬる抜けるドリブルが唯一無二。狭い局面でボールを失わない。" },
      { name: "フロリアン・ヴィルツ", position: "MF", club: "リバプール", why: "創造性の塊。一本のパスで局面を変える次世代の司令塔。" },
    ],
    trivia: [
      "優勝4回の強豪。『ドイツは強い』はサッカー界の格言。",
      "日本が2022年に逆転勝ちした相手。歴史的勝利の象徴。",
    ],
  },
  {
    code: "POR",
    name: "ポルトガル",
    enName: "Portugal",
    flag: "🇵🇹",
    group: "K",
    fifaRank: 5,
    themeColor: "#006600",
    nickname: "セレソン・ダス・キナス",
    tactics:
      "攻撃的なタレントが豊富で、サイド攻撃と個の打開力が武器。世代交代で若い才能が前線を活性化。",
    recent:
      "クリスティアーノ・ロナウドのラストダンスになるか注目。新世代との融合がカギ。",
    players: [
      { name: "クリスティアーノ・ロナウド", position: "FW", club: "アル・ナスル", why: "サッカー界のレジェンド。最後のW杯になるかもしれない一挙手一投足が見どころ。" },
      { name: "ラファエル・レオン", position: "FW", club: "ミラン", why: "規格外のスピードとパワー。乗ると手がつけられない。" },
    ],
    trivia: [
      "ロナウドの母国。彼の得点記録は国際試合で世界一。",
      "2016年にEURO初制覇。大会では勝負強さを発揮する。",
    ],
  },
  {
    code: "NED",
    name: "オランダ",
    enName: "Netherlands",
    flag: "🇳🇱",
    group: "F",
    fifaRank: 7,
    themeColor: "#ff6600",
    nickname: "オランエ",
    tactics:
      "『トータルフットボール』の発祥国。ポジションを流動的に入れ替える攻撃と、技術の高いビルドアップが伝統。",
    recent:
      "優勝経験はないが常に上位の実力国。『無冠の帝王』の歴史に終止符を打てるか。",
    players: [
      { name: "フィルジル・ファン・ダイク", position: "DF", club: "リバプール", why: "世界最高峰のセンターバック。彼の存在で守備が一枚岩になる。" },
      { name: "コーディ・ガクポ", position: "FW", club: "リバプール", why: "長身ながら技術もある万能アタッカー。左から仕掛けて決め切る。" },
    ],
    trivia: [
      "サポーターのオレンジ一色の応援は『オレンジ軍団』として有名。",
      "美しいサッカーで魅了するが優勝に届かない悲劇の歴史も人気の理由。",
    ],
  },
  {
    code: "USA",
    name: "アメリカ",
    enName: "USA",
    flag: "🇺🇸",
    group: "D",
    fifaRank: 16,
    themeColor: "#0a3161",
    nickname: "ザ・スターズ・アンド・ストライプス",
    tactics:
      "豊富な運動量とフィジカルをベースに、欧州で経験を積んだ若手が技術面を底上げ。開催国の後押しは大きな武器。",
    recent:
      "2026年共催の開催国。ホームの大声援を背に過去最高成績を狙う。サッカー人気も国内で急上昇。",
    players: [
      { name: "クリスチャン・プリシッチ", position: "FW", club: "ミラン", why: "アメリカの顔。ドリブルと得点力で開催国を牽引するスター。" },
    ],
    trivia: [
      "2026年大会の共催国（カナダ・メキシコと共催）。試合の多くが米国開催。",
      "国内ではアメフト・バスケに次ぐ人気だがW杯開催で熱が一気に高まる。",
    ],
  },
  {
    code: "MAR",
    name: "モロッコ",
    enName: "Morocco",
    flag: "🇲🇦",
    group: "C",
    fifaRank: 12,
    themeColor: "#c1272d",
    nickname: "アトラスの獅子",
    tactics:
      "堅守速攻の完成度が高く、組織的な守備ブロックから一気にカウンター。チームの一体感が武器。",
    recent:
      "本大会はグループCでブラジルと同組。2022年にアフリカ勢初のベスト4に輝いた勢いそのままに、強豪相手の再現に挑む。番狂わせの主役として世界が注目。",
    players: [
      { name: "アシュラフ・ハキミ", position: "DF", club: "パリ・サンジェルマン", why: "爆速で駆け上がる攻撃的サイドバック。守備的な選手なのに点に絡む派手さ。" },
    ],
    trivia: [
      "2022年のベスト4進出はアラブ・アフリカ全体の歴史的快挙として熱狂を呼んだ。",
      "選手が試合後に母親と喜ぶ姿が世界中で愛された。",
    ],
  },
  {
    code: "KOR",
    name: "韓国",
    enName: "South Korea",
    flag: "🇰🇷",
    group: "A",
    fifaRank: 23,
    themeColor: "#c8102e",
    nickname: "テグク戦士",
    tactics:
      "豊富な運動量と最後まで走り切る粘り強さが武器。欧州組のエースを軸に、前線からの圧力と縦への速さで勝負する。",
    recent:
      "本大会はグループAでメキシコと同組。開催国相手の開幕週から正念場。アジアの強豪として安定した出場記録を誇る。",
    players: [
      { name: "ソン・フンミン", position: "FW", club: "ロサンゼルスFC", why: "アジア最高峰のスター。左足の一撃と決定力は世界レベル。" },
      { name: "イ・ガンイン", position: "MF", club: "パリ・サンジェルマン", why: "技術と創造性で違いを生む司令塔。仕掛けが鋭い。" },
    ],
    trivia: [
      "2002年日韓W杯でベスト4。アジア勢の歴史的快挙を達成した。",
      "日本の永遠のライバル。『日韓戦』はアジア最大級の注目カード。",
    ],
  },
  {
    code: "SWE",
    name: "スウェーデン",
    enName: "Sweden",
    flag: "🇸🇪",
    group: "F",
    fifaRank: 24,
    themeColor: "#006aa7",
    nickname: "ブロー・ギュル（青と黄）",
    tactics:
      "屈強なフィジカルとセットプレーの強さが伝統。近年は前線に欧州得点王級のストライカーを擁し、縦に速い攻撃で一発の怖さがある。",
    recent:
      "本大会はグループFで日本と同組。プレーオフを勝ち上がっての出場で、長身FWの破壊力が日本にとって脅威に。",
    players: [
      { name: "アレクサンデル・イサク", position: "FW", club: "リバプール", why: "しなやかさと得点力を兼備するモダンな点取り屋。一瞬で試合を決める。" },
      { name: "ヴィクトル・ヨケレス", position: "FW", club: "アーセナル", why: "ゴリゴリ突進する『重戦車』型FW。当たり負けしない強さが魅力。" },
    ],
    trivia: [
      "イブラヒモビッチを生んだ国。長身でフィジカルの強い選手が多い。",
      "青と黄のユニフォームは国旗の色そのもの。",
    ],
  },
  {
    code: "TUN",
    name: "チュニジア",
    enName: "Tunisia",
    flag: "🇹🇳",
    group: "F",
    fifaRank: 41,
    themeColor: "#e70013",
    nickname: "鷲の戦士（カルタゴの鷲）",
    tactics:
      "規律ある守備ブロックとハードワークが身上。組織的に守ってカウンターを狙う、与しやすくない北アフリカの実力国。",
    recent:
      "本大会はグループFで日本と同組。2022年には王者フランスを撃破した実績もあり、油断できない相手。",
    players: [
      { name: "ハンニバル・メイブリ", position: "MF", club: "バーンリー（イングランド）", why: "闘志あふれる中盤の万能型。球際の強さでチームに勢いを呼ぶ。" },
    ],
    trivia: [
      "2022年大会でフランスを1-0で破る金星を挙げた。",
      "アフリカ屈指の安定した守備が持ち味。",
    ],
  },
  {
    code: "MEX",
    name: "メキシコ",
    enName: "Mexico",
    flag: "🇲🇽",
    group: "A",
    fifaRank: 14,
    themeColor: "#006847",
    nickname: "エル・トリ",
    tactics:
      "テクニカルな中盤と運動量で主導権を握るスタイル。熱狂的なホームの後押しを背に攻撃的に戦う。",
    recent:
      "2026年共催の開催国。開幕戦を本拠地で迎える。ベスト16の壁（『5試合目の呪い』）突破が悲願。",
    players: [
      { name: "サンティアゴ・ヒメネス", position: "FW", club: "ミラン", why: "決定力に優れる新世代のストライカー。開催国の得点源。" },
    ],
    trivia: [
      "開幕戦の舞台は伝統のアステカ・スタジアム。W杯を3度開催する初の国に。",
      "サポーターの熱量は中南米屈指。スタジアムが緑に染まる。",
    ],
  },
  {
    code: "CRO",
    name: "クロアチア",
    enName: "Croatia",
    flag: "🇭🇷",
    group: "L",
    fifaRank: 10,
    themeColor: "#ff0000",
    nickname: "ヴァトレニ（炎の男たち）",
    tactics:
      "中盤の技術と運動量で試合を支配。粘り強く、PK戦に滅法強い『トーナメントの鬼』。",
    recent:
      "本大会はグループLでイングランドと同組。2018年準決勝でイングランドを破った再戦が見もの。人口400万人の小国ながら2018年準優勝・2022年3位の常連。",
    players: [
      { name: "ルカ・モドリッチ", position: "MF", club: "ACミラン", why: "40歳でも世界トップの司令塔。年齢を超越した技術に驚く。" },
    ],
    trivia: [
      "赤白チェックのユニフォームは国旗のデザインそのもの。一目で分かる。",
      "小国が大国を倒し続ける『ジャイキリの代名詞』。",
    ],
  },
  {
    code: "CAN",
    name: "カナダ",
    enName: "Canada",
    flag: "🇨🇦",
    group: "B",
    fifaRank: 30,
    themeColor: "#D52B1E",
    nickname: "ザ・カナックス（赤き楓）",
    tactics:
      "ジェシー・マーシュ監督のもと前線から激しくプレスし、デイヴィスの爆速サイド突破とジョナサン・デイヴィッドの決定力で一気に縦へ仕掛けるトランジション型。",
    recent:
      "共催の開催国としてグループBに入り、スイス・ボスニア・ヘルツェゴビナ・カタールと同居。主軸デイヴィスがハムストリングの負傷を抱え開幕間に合うか不透明で、まずはカタール戦から勢いをつけたい立ち位置。",
    players: [
      { name: "アルフォンソ・デイヴィス", position: "左サイドバック／ウインガー", club: "バイエルン・ミュンヘン", why: "陸上選手級のスピードで左サイドを駆け上がる、カナダの顔にして世界屈指の快足アタッカー。" },
      { name: "ジョナサン・デイヴィッド", position: "フォワード", club: "ユヴェントス", why: "セリエの名門ユヴェントスへ移籍した点取り屋。冷静なフィニッシュでゴールを量産する大黒柱。" },
    ],
    trivia: [
      "国土面積は世界第2位で、東西の時差は最大4時間半にもなる広大な国。",
      "公用語は英語とフランス語の2つで、紙幣や標識も2言語併記が基本。",
    ],
  },
  {
    code: "URU",
    name: "ウルグアイ",
    enName: "Uruguay",
    flag: "🇺🇾",
    group: "H",
    fifaRank: 17,
    themeColor: "#0038A8",
    nickname: "ラ・セレステ（空色の闘将）",
    tactics:
      "ビエルサ監督らしい全員が走り続けるハイインテンシティのプレスと、ヴァルベルデを中心とした縦に速い攻撃。伝統の堅守「ガラ」精神も健在で、球際の強さは世界トップ級。",
    recent:
      "南米予選を4位通過し、グループHでスペイン・カーボベルデ・サウジアラビアと対戦。スアレスやカバーニが去り世代交代の途上にあり、ビエルサと選手の確執も一部で報じられる中、優勝2回の伝統国としてスペイン超えを狙う。",
    players: [
      { name: "フェデリコ・ヴァルベルデ", position: "ミッドフィルダー", club: "レアル・マドリード", why: "レアルの主将も任される万能司令塔。豪快なミドルシュートと無尽蔵のスタミナでピッチを支配する。" },
      { name: "ダルウィン・ヌニェス", position: "フォワード", club: "アル・ヒラル", why: "規格外のパワーとスピードを持つ点取り屋。良くも悪くも目が離せない決定機の主役。" },
    ],
    trivia: [
      "人口約350万人の小国ながらW杯優勝2回を誇る、人口あたり最強クラスのサッカー大国。",
      "牛肉の消費量が世界トップクラスで、炭火焼きの「アサード」が国民的ごちそう。",
    ],
  },
  {
    code: "COL",
    name: "コロンビア",
    enName: "Colombia",
    flag: "🇨🇴",
    group: "K",
    fifaRank: 13,
    themeColor: "#FCD116",
    nickname: "ロス・カフェテロス（黄色い珈琲戦士）",
    tactics:
      "ロレンソ監督のもと、ハメスの創造性とルイス・ディアスの個人技を最大化するテクニカルな攻撃サッカー。ボール保持から緩急をつけて崩し、リズミカルに相手を翻弄する。",
    recent:
      "南米予選を得失点差でウルグアイ・ブラジルの上に立つ好成績で突破し、FIFAランクは13位前後の有力国。グループKでポルトガル・コンゴ民主共和国・ウズベキスタンと同居し、ポルトガルとの上位争いがグループ首位の鍵になる。",
    players: [
      { name: "ルイス・ディアス", position: "ウインガー", club: "バイエルン・ミュンヘン", why: "バイエルンの左を切り裂く爆発力。ドリブルとスピードで一瞬にして試合を動かすエース。" },
      { name: "ハメス・ロドリゲス", position: "ミッドフィルダー", club: "ミネソタ・ユナイテッド（MLS）", why: "2014年W杯得点王の左足の魔術師。今も代表の主将としてゲームを操る司令塔。" },
    ],
    trivia: [
      "世界有数のコーヒー産地で、香り高いアラビカ種の名産地として知られる。",
      "故ガブリエル・ガルシア＝マルケスを生んだ文学の国で、陽気なサルサやクンビアも盛ん。",
    ],
  },
  {
    code: "PAR",
    name: "パラグアイ",
    enName: "Paraguay",
    flag: "🇵🇾",
    group: "D",
    fifaRank: 40,
    themeColor: "#D52B1E",
    nickname: "ラ・アルビロハ（赤白の堅守軍団）",
    tactics:
      "アルファロ監督が植え付けた堅い守備ブロックと組織力が土台。粘り強く守ってセットプレーとカウンターで仕留める、勝負強い現実的なサッカー。",
    recent:
      "アルファロ監督のもとブラジル・アルゼンチンから勝点を奪う躍進で南米予選6位通過、16年ぶりのW杯復帰を果たした。グループDでアメリカ・オーストラリア・トルコと対戦し、堅守を武器に久々の本大会で旋風を狙う立ち位置。",
    players: [
      { name: "ミゲル・アルミロン", position: "ミッドフィルダー", club: "アトランタ・ユナイテッド", why: "元ニューカッスルの快足アタッカー。スピードと運動量で攻撃を加速させる代表の顔。" },
      { name: "フリオ・エンシソ", position: "アタッカー", club: "ストラスブール", why: "予選3得点の若き才能。鋭いドリブルと一発のシュートを秘めた次世代のスター候補。" },
    ],
    trivia: [
      "公用語はスペイン語と先住民の言葉グアラニー語の2つで、日常会話にグアラニー語が深く根づく。",
      "南米のほぼ中央に位置する内陸国で、巨大なイタイプダムの水力発電で電力を大量に輸出している。",
    ],
  },
  {
    code: "ECU",
    name: "エクアドル",
    enName: "Ecuador",
    flag: "🇪🇨",
    group: "E",
    fifaRank: 23,
    themeColor: "#FFD100",
    nickname: "ラ・トリ（黄色い赤道戦士）",
    tactics:
      "ベッカセーセ監督のもと若く屈強な守備陣を土台に、カイセドが中盤を制圧するフィジカル＆運動量型。最終ラインの組織が安定し、堅守速攻で勝点を積み上げる。",
    recent:
      "南米予選をアルゼンチンに次ぐ2位で突破した好調ぶりで、欧州各国でプレーする若い世代が充実。グループEでドイツ・キュラソー・コートジボワールと同居し、強豪ドイツとの初戦が16強進出の試金石になる立ち位置。",
    players: [
      { name: "モイセス・カイセド", position: "ミッドフィルダー", club: "チェルシー", why: "チェルシーの中盤を支える鉄人。広い守備範囲とボール奪取力で相手の攻撃を根こそぎ刈り取る。" },
      { name: "ピエロ・インカピエ", position: "ディフェンダー", club: "アーセナル", why: "アーセナルへ加入した若き左利きCB。スピードと対人の強さを兼ね備えた次世代の守備の柱。" },
    ],
    trivia: [
      "国名はスペイン語で「赤道」の意味で、その名の通り国土の中央を赤道が通っている。",
      "独自の進化を遂げた生物で有名なガラパゴス諸島を擁し、世界遺産として知られる。",
    ],
  },
  {
    code: "SEN",
    name: "セネガル",
    enName: "Senegal",
    flag: "🇸🇳",
    group: "I",
    fifaRank: 14,
    themeColor: "#00853F",
    nickname: "テランガのライオン",
    tactics:
      "屈強なCBクリバリを軸にした堅守速攻が伝統。前線のイスマイラ・サールやニコラ・ジャクソンの推進力で一気に殴り切る。",
    recent:
      "2025年にAFCON王座を奪還し、FIFAランクで自国史上最高の12位前後まで上昇。グループIではフランスが頭一つ抜けるが、ノルウェー・イラクとの2位争いではアフリカ王者として有利な立場にある。",
    players: [
      { name: "サディオ・マネ", position: "FW", club: "アル・ナスル", why: "リバプール時代の英雄が34歳でラストダンス。決める時に決める男だ" },
      { name: "パプ・マタル・サール", position: "MF", club: "トッテナム", why: "中盤を走り回る若き心臓。攻守の切り替えの速さに注目" },
    ],
    trivia: [
      "「テランガ」は現地ウォロフ語で“おもてなし”を意味し、国民性そのものを表す言葉。",
      "首都ダカールは世界一過酷なラリー「ダカール・ラリー」の名前の由来になった街。",
    ],
  },
  {
    code: "CIV",
    name: "コートジボワール",
    enName: "Ivory Coast",
    flag: "🇨🇮",
    group: "E",
    fifaRank: 40,
    themeColor: "#FF8200",
    nickname: "エレファンツ（象）",
    tactics:
      "両ウイングの快足を生かしたサイド攻撃が持ち味で、奪ってから縦に速い。複数の得点源を持ち、誰からでも点が取れる多彩さが強み。",
    recent:
      "CAF予選では8勝2敗、得失点差プラス25でアフリカ最強の数字を残し無敗突破。グループEではドイツが本命だが、エクアドル・キュラソーとの2位争いには地元開催だった2023年AFCON王者の自信を持って臨む。",
    players: [
      { name: "アマド・ディアロ", position: "FW", club: "マンチェスター・ユナイテッド", why: "緩急自在のドリブラー。一瞬で局面を変えるキレ味が見どころ" },
      { name: "フランク・ケシエ", position: "MF", club: "アル・アハリ", why: "走って奪って攻め上がる万能型キャプテン。中盤の支配者だ" },
    ],
    trivia: [
      "国名はフランス語で“象牙海岸”の意味で、かつて象牙交易で栄えた歴史に由来する。",
      "世界一のカカオ生産国で、あなたが食べるチョコの多くはこの国産の豆かもしれない。",
    ],
  },
  {
    code: "GHA",
    name: "ガーナ",
    enName: "Ghana",
    flag: "🇬🇭",
    group: "L",
    fifaRank: 70,
    themeColor: "#CE1126",
    nickname: "ブラックスターズ",
    tactics:
      "屈強なフィジカルとセメンヨ・パーテイの推進力を軸に、縦に速く戦う。ベテラン監督ケイロスの下で守備の規律を整え、堅実さを増した。",
    recent:
      "ケイロス監督就任後にチームを立て直し本大会へ。攻撃の核クドゥスは負傷で離脱の見込みと報じられ、その穴をどう埋めるかが焦点。グループLはイングランドが本命で、クロアチア・パナマとの2位争いに挑む（ESPN報道）。",
    players: [
      { name: "アントワーヌ・セメンヨ", position: "FW", club: "マンチェスター・シティ", why: "馬力とスピードを兼ね備えた最前線。一人で違いを生める存在" },
      { name: "トーマス・パーテイ", position: "MF", club: "ビジャレアル", why: "中盤に君臨する司令塔。守備でも効くベテランの安定感に注目" },
    ],
    trivia: [
      "サハラ以南のアフリカで最初に独立を達成した国で、アフリカ独立運動の象徴的存在。",
      "国旗の黒い星はアフリカの自由と団結を表し、愛称「ブラックスターズ」の由来になっている。",
    ],
  },
  {
    code: "EGY",
    name: "エジプト",
    enName: "Egypt",
    flag: "🇪🇬",
    group: "G",
    fifaRank: 30,
    themeColor: "#C8102E",
    nickname: "ファラオたち",
    tactics:
      "絶対的エース、サラーを起点に攻撃を組み立てる。マルムーシュら新世代の台頭で前線の厚みが増し、サラー依存からの脱却を進めている。",
    recent:
      "2025年10月にジブチを破り出場決定。10戦8勝の無敗で堂々の首位突破を果たした。グループGはベルギーが本命で、イラン・ニュージーランドとの2位争いを制すれば悲願の初のベスト8が見えてくる。",
    players: [
      { name: "モハメド・サラー", position: "FW", club: "リバプール", why: "言わずと知れた世界的エース。代表通算得点記録更新も目前だ" },
      { name: "オマル・マルムーシュ", position: "FW", club: "マンチェスター・シティ", why: "今季ブレイクの快速FW。サラーの相棒として一気に脅威増" },
    ],
    trivia: [
      "ピラミッドやスフィンクスで知られる古代文明の地で、世界中の観光客を集める。",
      "国土の大半が砂漠で、人口の多くがナイル川沿いに集中して暮らしている。",
    ],
  },
  {
    code: "ALG",
    name: "アルジェリア",
    enName: "Algeria",
    flag: "🇩🇿",
    group: "J",
    fifaRank: 37,
    themeColor: "#006233",
    nickname: "砂漠の狐",
    tactics:
      "マフレズの個とアモウラの決定力を軸に、技巧的なパスワークで崩す。ペトコビッチ監督の下で組織的な守備とカウンターの精度を高めた。",
    recent:
      "予選10戦8勝1敗でウガンダに7差をつけ首位突破、12年ぶりの本大会復帰を果たした。アモウラが予選10得点でアフリカ得点王に輝き勢いに乗る。グループJはアルゼンチンが別格で、オーストリア・ヨルダンとの2位争いが焦点だ。",
    players: [
      { name: "リヤド・マフレズ", position: "FW", club: "アル・アハリ", why: "プレミア王者の左足。これがラストW杯と公言する円熟の技巧派" },
      { name: "モハメド・アモウラ", position: "FW", club: "ヴォルフスブルク", why: "予選得点王の快速FW。小柄ながら決定力抜群の新エース" },
    ],
    trivia: [
      "アフリカ大陸で最も国土が広い国で、その面積の大半をサハラ砂漠が占める。",
      "GKルカ・ジダンは仏代表の英雄ジネディーヌ・ジダンの息子で、アルジェリア代表を選んだ。",
    ],
  },
  {
    code: "SCO",
    name: "スコットランド",
    enName: "Scotland",
    flag: "🏴󠁧󠁢󠁳󠁣󠁴󠁿",
    group: "C",
    fifaRank: 38,
    themeColor: "#0065BF",
    nickname: "ザ・タータン・アーミー",
    tactics:
      "球際の強さと運動量で殴り続けるハードワーク型。中盤のマクトミネイ＋マギンが攻守の心臓で、左のロバートソンとタヴァニアが押し上げて厚みを作る。",
    recent:
      "デンマークに4-2で勝って28年ぶりの本大会を決め、勢いそのままに乗り込む。グループCはブラジルが頭一つ抜けるが、モロッコ・ハイチとの2位争いは射程圏内だ。",
    players: [
      { name: "スコット・マクトミネイ", position: "MF", club: "ナポリ", why: "マンU退団後にナポリで覚醒し優勝に貢献、突っ込んでくるボックス内のゴールが代名詞。" },
      { name: "アンディ・ロバートソン", position: "DF", club: "リヴァプール", why: "世界屈指の左サイドバックで、キャプテンとして駆け上がるクロスが武器。" },
    ],
    trivia: [
      "サッカーの近代ルールはイングランドと並びスコットランドが発祥に深く関わり、史上初の国際試合(1872年)も両国が戦った。",
      "ゴルフ発祥の地で、聖地セント・アンドルーズはこの国にある。",
    ],
  },
  {
    code: "HAI",
    name: "ハイチ",
    enName: "Haiti",
    flag: "🇭🇹",
    group: "C",
    fifaRank: 84,
    themeColor: "#00209F",
    nickname: "レ・グルナディエ（擲弾兵）",
    tactics:
      "屈強なFW陣の高さと推進力を活かし、堅守速攻で一発を狙うスタイル。前線のピエロ＝ナゾンの個の打開力に頼る場面が多い。",
    recent:
      "最終節でニカラグアを2-0で下しグループ首位通過、52年ぶり2度目の本大会へ。グループCではブラジルが格上で、スコットランド・モロッコ相手にどこまで食らいつけるかが焦点だ。",
    players: [
      { name: "フランツディ・ピエロ", position: "FW", club: "チャイクル・リゼスポル", why: "代表49試合32得点の長身ストライカー、高さと決定力でゴールをこじ開ける。" },
      { name: "ダッケンス・ナゾン", position: "FW", club: "エステグラル(イラン)", why: "8カ国13クラブを渡り歩いた歴代最多得点者、経験豊富な点取り屋。" },
    ],
    trivia: [
      "中南米初の黒人共和国として1804年に独立した、世界で初めて奴隷制を自力で打破した国。",
      "公用語はフランス語とハイチ・クレオール語で、活気あるコンパ(Kompa)という独自の音楽文化を持つ。",
    ],
  },
  {
    code: "PAN",
    name: "パナマ",
    enName: "Panama",
    flag: "🇵🇦",
    group: "L",
    fifaRank: 32,
    themeColor: "#D21034",
    nickname: "ラ・マレア・ロハ（赤い潮流）",
    tactics:
      "ゴドイ＆カラスキージャの中盤が試合をコントロールし、規律ある守備をベースに前線の個で仕留める。経験豊富なベテラン主体の堅実なチーム。",
    recent:
      "クリスチャンセン監督の下、予選を危なげなく勝ち抜き2度目の本大会へ。グループLではイングランドが本命で、クロアチア・ガーナとの2位争いに割って入れるかが勝負どころだ。",
    players: [
      { name: "アニバル・ゴドイ", position: "MF", club: "サンディエゴFC（MLS）", why: "代表約160キャップの司令塔キャプテン、CONCACAF屈指のゲームメーカー。" },
      { name: "アダルベルト・カラスキージャ", position: "MF", club: "プーマスUNAM", why: "中盤の底でバランスを取る潰し屋、攻守の切り替えで存在感を放つ。" },
    ],
    trivia: [
      "太平洋と大西洋を結ぶ「パナマ運河」を擁し、世界の海運の要衝として知られる。",
      "中米と南米をつなぐ陸の架け橋に位置し、多様な野生動物が暮らす生物多様性の宝庫。",
    ],
  },
  {
    code: "CUW",
    name: "キュラソー",
    enName: "Curaçao",
    flag: "🇨🇼",
    group: "E",
    fifaRank: 88,
    themeColor: "#002B7F",
    nickname: "ブルー・ウェーブ（青い波）",
    tactics:
      "オランダ流の技術と人工的に集めたタレントで、ボールを保持しながら戦う。チョンやバクーナ兄弟ら欧州育ちの個が攻撃を牽引する。",
    recent:
      "ジャマイカやバミューダを退け、史上最小の国としてまさかの初出場を決めた。グループEではドイツが格上で、コートジボワール・エクアドル相手に1ポイントでも奪えれば大金星だ。",
    players: [
      { name: "タヒス・チョン", position: "FW", club: "シェフィールド・ユナイテッド", why: "元マンU所属のドリブラー、左右に切れ込む技巧でチャンスを生む。" },
      { name: "ジュニーニョ・バクーナ", position: "MF", club: "フォレンダム", why: "中盤を司る兄弟プレーヤー、正確なキックで攻撃の起点になる。" },
    ],
    trivia: [
      "カリブ海に浮かぶ人口約15万人・面積443平方キロのオランダ王国構成国で、W杯出場国としては史上最小。",
      "鮮やかなパステルカラーの街並みと、青く透き通った海を持つリゾート地としても有名。",
    ],
  },
  {
    code: "NZL",
    name: "ニュージーランド",
    enName: "New Zealand",
    flag: "🇳🇿",
    group: "G",
    fifaRank: 86,
    themeColor: "#000000",
    nickname: "オール・ホワイツ",
    tactics:
      "長身選手を活かしたフィジカルとセットプレーが武器で、堅守を土台に戦う。エースのウッドめがけて速く正確にボールを送り込む。",
    recent:
      "OFC予選を圧倒し、地区から直接出場枠を得て初のプレーオフ回避で本大会へ。グループGではベルギーが本命だが、エジプト・イラン相手なら互角の勝負に持ち込めると見ている。",
    players: [
      { name: "クリス・ウッド", position: "FW", club: "ノッティンガム・フォレスト", why: "プレミアで活躍する大黒柱、予選9得点を量産したキャプテン兼点取り屋。" },
      { name: "マルコ・スタメニッチ", position: "ミッドフィルダー", club: "スウォンジー・シティ（イングランド）", why: "英2部スウォンジーでプレーする若き中盤の主力。豊富な運動量と展開力でチームの心臓を担う。" },
    ],
    trivia: [
      "サッカーよりラグビーが国民的スポーツで、代表「オールブラックス」と試合前の踊り「ハカ」が世界的に有名。",
      "羊の数が人口を大きく上回る自然豊かな国で、映画『ロード・オブ・ザ・リング』の撮影地としても知られる。",
    ],
  },
  {
    code: "SUI",
    name: "スイス",
    enName: "Switzerland",
    flag: "🇨🇭",
    group: "B",
    fifaRank: 19,
    themeColor: "#D52B1E",
    nickname: "ナティ（La Nati）",
    tactics:
      "規律ある守備ブロックと、ジャカを起点にした落ち着いたパス回しが持ち味。素早い切り替えからのカウンターで勝ち切る現実的なサッカー。",
    recent:
      "ベテラン主体でグループBに臨み、開催国カナダ、同居のボスニア・ヘルツェゴビナ、カタールと対戦する。実力的には突破の本命とされ、ベスト16以上が現実的な目標。",
    players: [
      { name: "グラニト・ジャカ", position: "MF", club: "サンダーランド（イングランド）", why: "代表144試合のキャプテン。ここに預ければボールが落ち着く、チームの心臓だよ。" },
      { name: "マヌエル・アカンジ", position: "DF", club: "インテル（イタリア）", why: "足元の上手い守備の要。ビルドアップから最終ラインまでこなす万能DF。" },
    ],
    trivia: [
      "永世中立国として知られ、国民投票で物事を決める直接民主制の国。",
      "公用語が4つ（独・仏・伊・ロマンシュ語）あり、代表にも多言語が飛び交う。",
    ],
  },
  {
    code: "BEL",
    name: "ベルギー",
    enName: "Belgium",
    flag: "🇧🇪",
    group: "G",
    fifaRank: 9,
    themeColor: "#E30613",
    nickname: "赤い悪魔（Red Devils）",
    tactics:
      "デ・ブライネの一本のパスとドク、トロサールら個の打開力で殴る攻撃型。世代交代の途上で、ベテランの質と若手の勢いを噛み合わせる。",
    recent:
      "ガルシア監督のもと、グループGでエジプト、イラン、ニュージーランドと対戦する。タレント力では格上で1位通過が濃厚、決勝トーナメントでどこまで勝ち上がれるかが問われる。",
    players: [
      { name: "ケビン・デ・ブライネ", position: "MF", club: "ナポリ（イタリア）", why: "世界屈指の司令塔。彼のスルーパス1本で試合が決まる、まさに魔法使い。" },
      { name: "ジェレミー・ドク", position: "FW", club: "マンチェスター・シティ（イングランド）", why: "止められないドリブラー。縦に仕掛ける突破は見ているだけでワクワクするよ。" },
    ],
    trivia: [
      "ベルギーはワッフル、フライドポテト発祥論、約1,500種ともいわれるビールで有名なグルメ大国。",
      "公用語はオランダ語・フランス語・ドイツ語の3つで、地域で言葉が分かれている。",
    ],
  },
  {
    code: "AUT",
    name: "オーストリア",
    enName: "Austria",
    flag: "🇦🇹",
    group: "J",
    fifaRank: 22,
    themeColor: "#ED2939",
    nickname: "ダス・チーム（Das Team）",
    tactics:
      "ラングニック仕込みの激しいハイプレスとゲーゲンプレッシングで主導権を握る。奪ってから一気にゴールへ向かう縦に速い攻撃が信条。",
    recent:
      "28年ぶりのW杯出場をつかみ、グループJでアルゼンチン、アルジェリア、ヨルダンと同居。アルゼンチンが頭一つ抜けるなか、2位の座をかけた争いが見どころになる。",
    players: [
      { name: "ダヴィド・アラバ", position: "DF", club: "レアル・マドリード（スペイン）", why: "DFも中盤もこなす万能の主将。レアルの一員という肩書きだけで凄さが伝わるね。" },
      { name: "マルセル・ザビッツァー", position: "MF", club: "ボルシア・ドルトムント（ドイツ）", why: "走って奪って点も取る中盤の万能型。ラングニック式プレスの心臓だ。" },
    ],
    trivia: [
      "モーツァルトやウィーン・フィルを生んだ「音楽の都」ウィーンを首都に持つ。",
      "ザッハトルテやアップルシュトゥルーデルなど、世界的に有名なスイーツ文化の国。",
    ],
  },
  {
    code: "NOR",
    name: "ノルウェー",
    enName: "Norway",
    flag: "🇳🇴",
    group: "I",
    fifaRank: 31,
    themeColor: "#BA0C2F",
    nickname: "ローベネ（Løvene／ライオンズ）",
    tactics:
      "ハーランドという規格外の点取り屋にボールを集める、シンプルかつ強烈な攻撃が魅力。エーデゴールが創造性を加え、縦に速く仕掛ける。",
    recent:
      "予選を全勝で駆け抜け、28年ぶりのW杯出場を達成した。グループIではフランス、セネガル、イラクと対戦し、優勝候補フランスとの初戦が大きな試金石となる。",
    players: [
      { name: "アーリング・ハーランド", position: "FW", club: "マンチェスター・シティ（イングランド）", why: "現代最強のストライカー。とにかく点を取る怪物、彼を見るだけで価値あり。" },
      { name: "マルティン・エーデゴール", position: "MF", club: "アーセナル（イングランド）", why: "アーセナル主将の司令塔。エレガントなパスでハーランドに餌を供給する。" },
    ],
    trivia: [
      "フィヨルドとオーロラで知られる北欧の国で、世界有数の幸福度の高さを誇る。",
      "産油国でありながら自然環境への意識が高く、EV普及率が世界トップクラス。",
    ],
  },
  {
    code: "CZE",
    name: "チェコ",
    enName: "Czech Republic",
    flag: "🇨🇿",
    group: "A",
    fifaRank: 41,
    themeColor: "#11457E",
    nickname: "ナーロダーク（Národák）",
    tactics:
      "屈強な中盤と高さを生かしたセットプレーが武器。シュビックの決定力に頼りつつ、堅実に守って一発を狙う粘り強いスタイル。",
    recent:
      "プレーオフでアイルランド、デンマークをいずれもPK戦で破り、20年ぶりのW杯出場を勝ち取った。グループAでは開催国メキシコ、南アフリカ、韓国と対戦し、突破を狙う。",
    players: [
      { name: "パトリック・シュビック", position: "FW", club: "バイヤー・レバークーゼン（ドイツ）", why: "ブンデスで量産する点取り屋。EUROのロングシュート伝説でも有名な男だ。" },
      { name: "トーマス・ソウチェク", position: "MF", club: "ウェストハム（イングランド）", why: "長身で攻守に効く中盤の主。セットプレーでヘディング弾も決める頼れる柱。" },
    ],
    trivia: [
      "首都プラハは「百塔の街」と呼ばれる美しい古都で、世界有数の観光地。",
      "一人当たりのビール消費量が世界一として知られる、筋金入りのビール大国。",
    ],
  },
  {
    code: "BIH",
    name: "ボスニア・ヘルツェゴビナ",
    enName: "Bosnia and Herzegovina",
    flag: "🇧🇦",
    group: "B",
    fifaRank: 74,
    themeColor: "#002F6C",
    nickname: "ズマイェヴィ（Zmajevi／龍たち）",
    tactics:
      "ジェコを最前線に置き、その高さとポストプレーを起点に攻める。守ってはハードワークで耐え、ベテランの経験で試合を締める現実路線。",
    recent:
      "プレーオフでウェールズ、イタリアを連続PK戦で破る大金星で本大会出場を決めた。グループBでは開催国カナダ、スイス、カタールと対戦し、初の決勝T進出を目指す。",
    players: [
      { name: "エディン・ジェコ", position: "FW", club: "シャルケ04（ドイツ）", why: "40歳でもまだ点を取る生ける伝説。元マンCの大エースに注目だよ。" },
      { name: "セアド・コラシナツ", position: "DF", club: "アタランタ（イタリア）", why: "屈強な左サイドの守備職人。2014年組の生き残りで、守りの精神的支柱。" },
    ],
    trivia: [
      "国民食ćevapi（チェヴァピ）など、トルコ文化の影響を受けた料理が名物。",
      "首都サラエボは冬季五輪の開催地で、東西文化が交わる独特の街並みを持つ。",
    ],
  },
  {
    code: "RSA",
    name: "南アフリカ",
    enName: "South Africa",
    flag: "🇿🇦",
    group: "A",
    fifaRank: 56,
    themeColor: "#007749",
    nickname: "バファナ・バファナ（少年たち）",
    tactics:
      "ベルギー人ブロース監督の規律ある守備をベースに、ボールを持てる選手の技巧で前進する。国内組中心ながら連動した攻守の切り替えが武器。",
    recent:
      "2010年の自国開催以来16年ぶり、自力では初の本大会出場を勝ち取った。グループAでは開幕戦で開催国メキシコと激突し、韓国・チェコと2枠を争う構図になる。",
    players: [
      { name: "ライル・フォスター", position: "FW", club: "バーンリー（イングランド）", why: "プレミア勢の点取り屋で、最前線を1人で背負うエース。" },
      { name: "テンバ・ズワネ", position: "MF", club: "ママロディ・サンダウンズ（南アフリカ）", why: "ボール扱いの巧さでチームに知性を加えるベテラン司令塔。" },
    ],
    trivia: [
      "公用語が11もある「虹の国」で、国歌は複数言語が混ざった珍しい構成。",
      "2010年大会でおなじみの大音量ラッパ「ブブゼラ」発祥の地。",
    ],
  },
  {
    code: "CPV",
    name: "カーボベルデ",
    enName: "Cape Verde",
    flag: "🇨🇻",
    group: "H",
    fifaRank: 70,
    themeColor: "#003893",
    nickname: "ブルーシャークス（青いサメ）",
    tactics:
      "組織された堅守からの速い切り替えで勝負する。欧州主要リーグ組は少ないが、ディアスポラ（移民系）の経験豊富な選手が技術と勝負強さを持ち込む。",
    recent:
      "人口約50万人の島国が10試合中8勝でカメルーンらを抑え、史上初のW杯出場を達成した。死の組とも言えるグループHでスペイン・ウルグアイ・サウジアラビアと戦う、大会最小級の国の挑戦だ。",
    players: [
      { name: "ロガン・コスタ", position: "DF", club: "ビジャレアル（スペイン）", why: "唯一の欧州5大リーグ組で、最終ラインを束ねる頼れる守備の柱。" },
      { name: "ライアン・メンデス", position: "FW", club: "ウードゥル（トルコ）", why: "代表最多出場・最多得点を誇る主将で、歴史的初陣を背負う顔。" },
    ],
    trivia: [
      "西アフリカ沖の大西洋に浮かぶ10の島からなる、人口50万人ほどの小さな島国。",
      "哀愁ある音楽ジャンル「モルナ」発祥の地で、歌姫セザリア・エヴォラを生んだ。",
    ],
  },
  {
    code: "COD",
    name: "コンゴ民主共和国",
    enName: "DR Congo",
    flag: "🇨🇩",
    group: "K",
    fifaRank: 60,
    themeColor: "#007FFF",
    nickname: "レオパード（豹）",
    tactics:
      "デサーブル監督のもと、身体能力を生かした強度の高い守備と縦に速い攻撃を志向する。プレミアリーグ組を軸に、若さと経験を融合させた攻撃的なスタイル。",
    recent:
      "予選はセネガルに次ぐ2位から大陸間プレーオフを勝ち抜き、ジャマイカを1-0で下して本大会切符をつかんだ。グループKではポルトガル、コロンビア、ウズベキスタンと同居する厳しい組だ。",
    players: [
      { name: "ヨアヌ・ウィサ", position: "FW", club: "ニューカッスル（イングランド）", why: "スピードと決定力を備えたプレミアの点取り屋で、最大の脅威。" },
      { name: "シャンセル・ムベンバ", position: "DF", club: "リール（フランス）", why: "代表最多出場の主将で、プレーオフで決勝弾を沈めた守備の絶対的支柱。" },
    ],
    trivia: [
      "アフリカ屈指の鉱物資源国で、スマホに不可欠なコバルト産出量は世界最大級。",
      "ルンバの源流の一つとされる音楽文化を持ち、UNESCO無形文化遺産にも登録された。",
    ],
  },
  {
    code: "UZB",
    name: "ウズベキスタン",
    enName: "Uzbekistan",
    flag: "🇺🇿",
    group: "K",
    fifaRank: 57,
    themeColor: "#1EB53A",
    nickname: "ホワイトウルブズ（白い狼）",
    tactics:
      "イタリア人カンナバーロ監督が整備した堅い守備組織が土台。マンC所属のフサノフを中心に締め、中盤の運動量と両翼の仕掛けでチャンスを作る現実的なスタイル。",
    recent:
      "2025年6月のUAE戦引き分けで、中央アジア初・史上初のW杯出場を1試合を残して決めた。グループKではポルトガル、コロンビア、コンゴ民主共和国と争い、初戦は6月18日にコロンビアと対戦する。",
    players: [
      { name: "アブドゥコディル・フサノフ", position: "DF", club: "マンチェスター・シティ（イングランド）", why: "ウズベク初のプレミア組で、スピードと対人で世界級の守備の柱。" },
      { name: "エルドル・ショムロドフ", position: "FW", club: "イスタンブール・バシャクシェヒル（トルコ）", why: "セリエAでも実績を残した主将のエースFW。" },
    ],
    trivia: [
      "世界に2つしかない「二重内陸国（海に出るまで2国またぐ）」の一つ。",
      "米料理「プロフ」が国民食で、サマルカンドなどシルクロードの古都を抱える。",
    ],
  },
  {
    code: "AUS",
    name: "オーストラリア",
    enName: "Australia",
    flag: "🇦🇺",
    group: "D",
    fifaRank: 26,
    themeColor: "#00843D",
    nickname: "サッカルーズ（カンガルー)",
    tactics:
      "ポポヴィッチ監督のもと、規律ある守備と球際の強さを軸に手堅く戦う。フィジカルとセットプレーを生かしつつ、ボイルら両翼の推進力で攻め切るスタイル。",
    recent:
      "アジア最終予選で日本に次ぐ2位を確保し、6大会連続のW杯出場を決めた。グループDでは開催国アメリカ、パラグアイ、トルコと対戦し、組2位以内を狙える位置にいる。",
    players: [
      { name: "マーティン・ボイル", position: "FW", club: "ハイバーニアン（スコットランド）", why: "スピードと突破力でサイドを切り裂く、攻撃の推進役。" },
      { name: "ミッチェル・デューク", position: "FW", club: "マカーサーFC（豪州）", why: "Jリーグでもおなじみ、空中戦に強い経験豊富な9番候補。" },
    ],
    trivia: [
      "もともとはオセアニア所属で、強くなりすぎて2006年からアジア連盟へ移籍した珍しい経歴を持つ。",
      "国土は日本の約20倍と広大だが、人口は約2600万人とそこまで多くない。",
    ],
  },
  {
    code: "KSA",
    name: "サウジアラビア",
    enName: "Saudi Arabia",
    flag: "🇸🇦",
    group: "H",
    fifaRank: 58,
    themeColor: "#005430",
    nickname: "緑の鷹（グリーンファルコンズ）",
    tactics:
      "国内リーグ組を主体にした堅守速攻型で、規律ある守備から手数をかけずに前線へ運ぶ。2022年にアルゼンチンを破った「縦に速い」スタイルが基本路線。",
    recent:
      "ドニス新監督のもと、ロシア大会から3大会連続出場を国史上初めて達成。グループHではスペインが頭一つ抜けるなか、カーボベルデやウルグアイとの2位争いに食い込めるかが焦点。",
    players: [
      { name: "サレム・アル・ドサリ", position: "FW（左ウイング）", club: "アル・ヒラル", why: "2022年にアルゼンチンから決勝点を奪った主将。左足の強烈なカットインが代名詞で、彼が爆発すれば何かが起きる。" },
      { name: "サウド・アブドゥルハミド", position: "DF（右サイドバック）", club: "ランス（仏・ローマから移籍）", why: "欧州フランス1部でプレーする数少ない海外組。攻め上がりの推進力でサイドを活性化する若き顔。" },
    ],
    trivia: [
      "週末の金曜が休日で、国の祝日「建国記念日（9/23）」には街中が緑一色に染まる。",
      "世界最大級の石油会社サウジアラムコを擁し、近年はサッカーへの巨額投資で世界的スターを次々と国内リーグに集めている。",
    ],
  },
  {
    code: "QAT",
    name: "カタール",
    enName: "Qatar",
    flag: "🇶🇦",
    group: "B",
    fifaRank: 53,
    themeColor: "#8A1538",
    nickname: "アル・アンナービ（えんじ色軍団）",
    tactics:
      "ボールを保持して崩すポゼッション志向で、アジア王者の自信を背景に主導権を握りにいく。アフィフを起点とした左サイドからの仕掛けが生命線。",
    recent:
      "ロペテギ監督のもとUAEを破り、史上初めて予選を勝ち抜いての出場を決めた。グループBはスイス、カナダ、ボスニア・ヘルツェゴビナと拮抗し、初の決勝トーナメント進出が現実味を帯びる。",
    players: [
      { name: "アクラム・アフィフ", position: "FW（左ウイング）", club: "アル・サッド", why: "アジア最優秀選手2度の絶対的エース。予選で大会最多11アシストを記録した司令塔で、彼のラストパスから多くが生まれる。" },
      { name: "アルモエズ・アリ", position: "FW（センターフォワード）", club: "アル・ドゥハイル（カタール）", why: "アジア予選得点王（12点）の点取り屋。アフィフとのホットラインでゴールを量産する決定力が見どころ。" },
    ],
    trivia: [
      "国土は秋田県ほどの広さながら世界有数の天然ガス埋蔵量を誇り、一人あたりGDPは世界トップクラスの富裕国。",
      "2022年に中東で初めてW杯を開催したばかりで、真夏の酷暑を避けるため史上初の冬開催となった。",
    ],
  },
  {
    code: "IRN",
    name: "イラン",
    enName: "Iran",
    flag: "🇮🇷",
    group: "G",
    fifaRank: 18,
    themeColor: "#239F40",
    nickname: "チーム・メリ（国民の代表）",
    tactics:
      "屈強なフィジカルと組織的な堅守をベースに、前線のタレミへ預けて起点を作る。アジアでは屈指の安定感を誇り、守ってカウンターの形が成熟している。",
    recent:
      "アジア予選を危なげなく突破し、ガレノイ監督が30人を発表。一方でアズムンが招集外となり波紋を呼んだ。グループGではベルギーが本命だが、エジプト、ニュージーランドを相手に十分に勝ち上がりを狙える。",
    players: [
      { name: "メフディ・タレミ", position: "FW（センターフォワード）", club: "オリンピアコス（ギリシャ）", why: "代表通算100キャップ超の大黒柱で3度目のW杯。欧州で揉まれた決定力とポストプレーがチームの生命線。" },
      { name: "アリレザ・ジャハンバフシュ", position: "FW（ウイング）", club: "FCVデンダー（ベルギー）", why: "元ブライトンの実力派ウインガー。経験豊富なドリブルとセットプレーで攻撃に変化をつける。" },
    ],
    trivia: [
      "正式名称はイラン・イスラム共和国で、公用語は英語やアラビア語ではなくペルシャ語。詩と絨毯（ペルシャ絨毯）の文化が世界的に名高い。",
      "暦はイスラム暦ではなくペルシャ暦を使い、春分の日にあたる新年「ノウルーズ」を盛大に祝う。",
    ],
  },
  {
    code: "IRQ",
    name: "イラク",
    enName: "Iraq",
    flag: "🇮🇶",
    group: "I",
    fifaRank: 58,
    themeColor: "#CE1126",
    nickname: "ライオンズ・オブ・メソポタミア（二河の獅子）",
    tactics:
      "規律と球際の強さを土台にした粘り強いサッカーで、海外組と国内組が融合する。大舞台での集中力と勝負強さが持ち味で、土壇場の勝負弱さを克服してきた。",
    recent:
      "アジア予選プレーオフでUAEを破り、3月の大陸間プレーオフでボリビアを2-1で下して悲願の出場を決めた。グループIは強豪フランスが頭一つ抜け、セネガル、ノルウェーとの争いで存在感を示せるかが課題。",
    players: [
      { name: "アイメン・フセイン", position: "FW（センターフォワード）", club: "国内・湾岸クラブ系", why: "予選で得点を重ねた頼れる点取り屋。高さと強さを生かしたターゲットマンとして攻撃の軸を担う。" },
      { name: "ザイダン・イクバル", position: "MF（中盤）", club: "ユトレヒト系（元マンU）", why: "元マンチェスター・ユナイテッド育成出身の技巧派。欧州仕込みの落ち着いたボールさばきで中盤を操る。" },
    ],
    trivia: [
      "ティグリス・ユーフラテス川に挟まれた地は「メソポタミア文明」発祥の地で、人類最古の都市文明が栄えた歴史を持つ。",
      "ナツメヤシ（デーツ）の一大産地で、甘い実は古くから国民の食卓に欠かせない存在となっている。",
    ],
  },
  {
    code: "JOR",
    name: "ヨルダン",
    enName: "Jordan",
    flag: "🇯🇴",
    group: "J",
    fifaRank: 62,
    themeColor: "#007A3D",
    nickname: "アル・ニシュール（鷲たち）",
    tactics:
      "堅い守備を土台に、アル・タマリとオルワンの俊足アタッカーで一気に仕留めるカウンターが看板。組織的な守りと前線の個の融合で勝ち点を積み上げてきた。",
    recent:
      "アジア予選を勝ち抜き、悲願の初出場を達成。セラミ監督が30人を発表した。グループJはアルゼンチンが断トツの本命で、アルジェリア、オーストリアとの2位争いに挑む立場だ。",
    players: [
      { name: "ムーサ・アル・タマリ", position: "FW（右ウイング）", club: "レンヌ（フランス）", why: "「ヨルダンのメッシ」と呼ばれる至宝。フランス1部で活躍する快足ドリブラーで、彼の突破が初出場国の希望そのもの。" },
      { name: "アリ・オルワン", position: "FW（センターフォワード）", club: "湾岸クラブ系", why: "予選でハットトリックを決め出場を引き寄せた点取り屋。アル・タマリとの2トップで火力を生む。" },
    ],
    trivia: [
      "世界遺産「ペトラ遺跡」を擁し、岩を削った神殿は映画のロケ地としても世界的に有名。",
      "国土の大半が砂漠だが、塩分濃度が極めて高く体が浮く「死海」を抱える観光大国でもある。",
    ],
  },
  {
    code: "TUR",
    name: "トルコ",
    enName: "Türkiye",
    flag: "🇹🇷",
    group: "D",
    fifaRank: 26,
    themeColor: "#E30A17",
    nickname: "アイ・ユルディズ（月と星）",
    tactics:
      "ギュレルとユルディズという若き才能を起点にした攻撃的なポゼッション。技巧と推進力を兼ね備え、モンテッラ監督のもとで攻守に整理された現代的なスタイルを志向する。",
    recent:
      "予選でスペインに次ぐ2位、プレーオフでルーマニア・コソボを退け、24年ぶりの本大会復帰を決めた。グループDはアメリカ（開催国）と拮抗し、パラグアイ、オーストラリアを相手に上位進出を狙える好メンバーだ。",
    players: [
      { name: "アルダ・ギュレル", position: "MF（攻撃的MF）", club: "レアル・マドリード（スペイン）", why: "レアルの背番号を背負う超新星。左足から繰り出す芸術的なパスとシュートは、世界中のファンが見逃せない逸材。" },
      { name: "ケナン・ユルディズ", position: "FW（セカンドトップ）", club: "ユヴェントス（イタリア）", why: "ユヴェントスの背番号10を継ぐ点取り屋。ドリブルと決定力を兼ね備え、欧州が熱視線を送る次世代エース。" },
    ],
    trivia: [
      "アジアとヨーロッパの両大陸にまたがる唯一の国で、イスタンブールの街は海峡を挟んで二つの大陸に分かれている。",
      "「ドネルケバブ」発祥の地として知られ、食文化はトルコ料理として世界三大料理の一つに数えられる。",
    ],
  },
];

// 観戦前ブリーフィングをマージ（時事情報は別ファイルで管理）
for (const t of teams) {
  const b = briefings[t.code];
  if (b) {
    t.whyWatch = b.whyWatch;
    t.briefing = b.briefing;
  }
}

export function getTeam(code: string): Team | undefined {
  return teams.find((t) => t.code === code);
}
